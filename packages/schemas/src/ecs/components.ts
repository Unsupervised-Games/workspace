...
import {
  DayNightPhaseSchema,
  PhaseThresholdsSchema,
  ScheduledHandleSchema,
} from '../data/time.js';
...
export const ConvexHullColliderSchema = z.object({
  ...
  points: z
    ...
    .refine((p) => p.length % 3 === 0 && p.length >= 12, {
      message:
        'convex-hull points must be a flat [x,y,z,...] array with at least 4 vertices (length divisible by 3, >= 12)',
    }),
});
...
export const CompoundChildConvexHullSchema = z.object({
  ...
  points: z
    ...
    .refine((p) => p.length % 3 === 0 && p.length >= 12, {
      message:
        'convex-hull points must be a flat [x,y,z,...] array with at least 4 vertices',
    }),
  localOffset: Vec3Schema,
  localRotation: QuaternionSchema,
});
...
export type CharacterControllerAutostep = z.infer<
  typeof CharacterControllerAutostepSchema
>;
