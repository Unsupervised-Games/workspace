// Narrative card schemas — the canonical narrative-entry types
// authored in narrative projects.
//
// A "card" is the atomic unit of a narrative project: one
...
// in `apps/workbench/lib/narrative/cardStorage.ts`.
//
...
const CardBaseSchema = z.object({
  ...
  id: z.string().min(1).regex(/^[a-z]+:[a-z0-9-]+$/, {
    message:
      "id must be of form 'type:slug' (lowercase letters, digits, hyphens)",
  }),
   ...
   *  without re-sniffing the extension. The picker in narrative
...
export const SUBTYPES_BY_TYPE = {
  character: [] as const,
  location: ['city', 'town', 'landmark', 'country'] as const,
  faction: ['company', 'agency', 'gang', 'guild', 'organization'] as const,
  item: ['weapon', 'vehicle', 'device', 'document'] as const,
  event: ['incident', 'meeting', 'festival'] as const,
  lore: ['history', 'culture', 'law', 'technology'] as const,
  ecology: ['flora', 'fauna'] as const,
} as const satisfies Record<CardType, readonly string[]>;
