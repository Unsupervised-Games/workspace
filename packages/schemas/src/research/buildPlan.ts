...
export type WorkItemClassification = z.infer<
  typeof WorkItemClassificationSchema
>;
...
export function parseBuildPlan(raw: unknown): BuildPlan {
  ...
  for (const item of parsed.workItems) {
    for (const dep of item.dependsOn) {
      if (!ids.has(dep)) {
        throw new Error(
          `WorkItem '${item.id}' depends on unknown work item '${dep}'.`,
        );
      }
    }
    if (item.round < 1) {
      throw new Error(
        `WorkItem '${item.id}' has invalid round ${item.round}.`,
      );
    }
    if (
      (item.classification === 'package' ||
        item.classification === 'extend-existing') &&
      item.skillOutcome !== null
    ) {
      throw new Error(
        `WorkItem '${item.id}' is classified '${item.classification}' but carries a skillOutcome. Package + extend-existing items use packageOutcome.`,
      );
    }
    if (
      item.classification === 'game-code' &&
      item.packageOutcome !== null
    ) {
      throw new Error(
        `WorkItem '${item.id}' is classified 'game-code' but carries a packageOutcome. Game-code items use skillOutcome.`,
      );
    }
    if (item.coveredCapabilityIds.length === 0) {
      throw new Error(
        `WorkItem '${item.id}' has no coveredCapabilityIds; every work item must cover ≥1 capability.`,
      );
    }
    if (
      item.classification !== 'package' &&
      ...
      throw new Error(
        `WorkItem '${item.id}' is classified '${item.classification}' but covers ${item.coveredCapabilityIds.length} capabilities. Only 'package' + 'extend-existing' items may collapse multiple capabilities.`,
      );
    }
  }
  return parsed;
