...
export type CapabilityClassification = z.infer<
  typeof CapabilityClassificationSchema
>;
...
export function parseFeatureDecomposition(
  raw: unknown,
  ...
  for (const c of parsed.capabilities) {
    for (const dep of c.dependsOn) {
      if (!capabilityIds.has(dep)) {
        throw new Error(
          `Capability '${c.id}' depends on unknown capability '${dep}'.`,
        );
      }
    }
    if (
      c.classification === 'extend-existing' &&
      c.extensionTarget === undefined
    ) {
      throw new Error(
        `Capability '${c.id}' is classified 'extend-existing' but has no extensionTarget. Set extensionTarget with packageId + extensionSketch.`,
      );
    }
  }
  for (const a of parsed.assertions) {
    if (!capabilityIds.has(a.capabilityId)) {
      throw new Error(
        `Assertion '${a.id}' references unknown capability '${a.capabilityId}'.`,
      );
    }
    for (const sid of a.derivedFromSources) {
      if (!sourceIds.has(sid)) {
        throw new Error(
          `Assertion '${a.id}' cites unknown source '${sid}'.`,
        );
      }
    }
  }
  for (const sketch of parsed.apiSketches) {
    for (const cid of sketch.providesCapabilities) {
      if (!capabilityIds.has(cid)) {
        throw new Error(
          `API sketch '${sketch.proposedPackageId}' references unknown capability '${cid}'.`,
        );
      }
    }
    for (const aid of sketch.assertionsRequired) {
      if (!assertionIds.has(aid)) {
        throw new Error(
          `API sketch '${sketch.proposedPackageId}' references unknown assertion '${aid}'.`,
        );
      }
    }
  }
  for (const s of parsed.citedSources) {
    for (const ref of s.supports) {
      if (
        !capabilityIds.has(ref) &&
        !assertionIds.has(ref)
      ) {
        throw new Error(
          `Source '${s.id}' supports unknown target '${ref}'.`,
        );
      }
    }
  }
